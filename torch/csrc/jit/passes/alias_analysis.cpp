...
void AliasDb::analyzeFork(Node* node) {
  ...
  // analysis is flow-insensitive, we can just assume that it runs here.
  const auto subgraph = node->g(attr::Subgraph).get();
  subgraphToOwner_.insert({subgraph, node});
  ...
  for (const auto& use : fut->uses()) {
    ...
    //
    // XXX: propagating the writes to every `wait` disallows some
    // optimizations, since the live interval ends at the *first* `wait`. We
    // should be able to arbitrarily reorder around subsequent `wait` calls,
    // since they are effectively no-ops.
